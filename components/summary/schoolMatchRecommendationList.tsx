...
interface SchoolMatchRecommendationListProps {
  generation: string;
  starredStates: { [key: number]: boolean };
  onToggleStarred: (index: number) => void;
}

export function SchoolMatchRecommendationList({
  generation,
  starredStates,
  onToggleStarred,
}: SchoolMatchRecommendationListProps) {
  return (
                ...
                <CardBody className="overflow-visible py-2 px-4">
                  <div className="space-y-3">
                    <div>
                      ...
                      <p className="text-sm">{option.descriptionOfCollege}</p>
                    </div>
                    <div>
                      <h5 className="font-semibold text-sm text-muted-foreground mb-1">
                        Why This College:
                      </h5>
                      <p className="text-sm">{option.whyThisCollege}</p>
                    </div>
                  </div>
                </CardBody>
              </Card>
